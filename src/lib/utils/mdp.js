...
class Mdp {
	...
	valueCalculation(state, action) {
		...
		for (let i = 0; i < transitionMatrix.length; i++) {
			for (let j = 0; j < transitionMatrix[i].length; j++) {
				let next_state = { x: state.x + i - 1, y: state.y + j - 1 };
				if (
					next_state.x < 0 ||
					next_state.x >= this.width ||
					next_state.y < 0 ||
					next_state.y >= this.height
				) {
					continue;
				}
	...
	valueIterationStep() {
		...
		for (let i = 0; i < this.width; i++) {
			for (let j = 0; j < this.height; j++) {
				let state = { x: i, y: j };
				let old_value = this.values[i][j].value;
				...
				max_value = this.grid[state.x][state.y].value + this.gamma * max_value;
				this.values[i][j] = { value: max_value, action: max_action };
